...
*
* java_io project is a simple demo of Java IO APIs
* 
...
public class Dir {
	public static void main(String[] args) {
		if (args.length == 0){
			System.out.println("Usage: java dir <path_to_directory> [regular_expression_filename_filter>]");
			return;
		}
		...
		File[] list = path.listFiles( (File dir, String name) -> pattern.matcher(name).matches() );
	
		//Results custom sorting - directories first lexicographical, after them files by size
		Arrays.sort(list, (File f1, File f2) -> {
			if(f1.isDirectory()){
				if(!f2.isDirectory())
					return -1;
				else
		...
		for (int i = 0; i < list.length; i++)
			System.out.printf("%60s %20s%n", list[i].getName(), list[i].length());
	}
}